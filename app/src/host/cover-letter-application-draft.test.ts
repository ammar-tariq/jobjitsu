...
describe("generateApplicationCoverLetterWithAi (PE08-S02)", () => {
  it("writes an editable cover letter and updates without sending", async () => {
    ...
    const result = await generateApplicationCoverLetterWithAi({
    ...
    expect(result.coverLetterStatus).toBe("ready");
    expect(result.draftText).toMatch(/Cover letter draft/i);
    expect(result.application?.coverLetterDraftText).toBe(result.draftText);
  ...
  it("returns calm unavailable when Agent is not ready", async () => {
    ...
    expect(result.coverLetterStatus).toBe("unavailable");
    expect(result.draftText).toBe("");
    expect(result.application?.stage).toBe("discover");
  });

  it("never invokes send when complete succeeds (fence: cover letter ↛ send)", async () => {
    const repository = createMemoryApplicationRepository();
    const created = await createApplicationDraft({
      repository,
