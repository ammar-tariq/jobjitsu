# Architecture Index

Design docs for JobJitsu — the AI Career Operating System.

| Doc | Topic |
|-----|--------|
| [SYSTEM_ARCHITECTURE.md](./SYSTEM_ARCHITECTURE.md) | System map (C4, layers, runtime paths) |
| [OVERVIEW.md](./OVERVIEW.md) | Thesis, laws, module mapping |
| [MONOREPO.md](./MONOREPO.md) | Repository structure |
| [PACKAGE_BOUNDARIES.md](./PACKAGE_BOUNDARIES.md) | Package ownership & dependency rules |
| [EVENT_SYSTEM.md](./EVENT_SYSTEM.md) | Local domain events |
| [PLUGIN_ARCHITECTURE.md](./PLUGIN_ARCHITECTURE.md) | Agent skills |
| [EXTENSION_SYSTEM.md](./EXTENSION_SYSTEM.md) | Host contributions |
| [DESKTOP_ARCHITECTURE.md](./DESKTOP_ARCHITECTURE.md) | Shell, IPC, UI |
| [AI_ARCHITECTURE.md](./AI_ARCHITECTURE.md) | Local Intelligence + workflow engine contract |
| [DATA_MODELS.md](./DATA_MODELS.md) | Entity ownership & schemas |
| [TAURI_TS_RUNTIME.md](./TAURI_TS_RUNTIME.md) | Tauri ↔ TypeScript shell runtime |
| [SCHEDULER.md](./SCHEDULER.md) | Local jobs & follow-ups |
| [TESTING_STRATEGY.md](./TESTING_STRATEGY.md) | Privacy & quality bars |

**Terminology:** [../product/TERMINOLOGY.md](../product/TERMINOLOGY.md)

Product & brand anchors: [../product/PRODUCT_VISION.md](../product/PRODUCT_VISION.md) · [../brand/PRODUCT_PHILOSOPHY.md](../brand/PRODUCT_PHILOSOPHY.md)
